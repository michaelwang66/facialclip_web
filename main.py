...
def computer_uar_war(val_loader, model, best_checkpoint_path, log_confusion_matrix_path, log_txt_path, data_set):
    #  Force map_location and weights_only=False to resolve safe loading and device mismatch issues
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    checkpoint = torch.load(best_checkpoint_path, map_location=device, weights_only=False)
    pre_trained_dict = checkpoint['state_dict']
    
    #  Handle 'module.' prefix generated by DataParallel
    new_state_dict = {k.replace('module.', ''): v for k, v in pre_trained_dict.items()}
    
    #  Make sure you load into the correct submodel
